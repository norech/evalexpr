...
{
    cr_expect_eq(eval_expr("2*(1+1)"), 4);
}

Test (eval_expr, brackets_times_two)
{
    cr_expect_eq(eval_expr("(1+1)*2"), 4);
}

Test (eval_expr, brackets_times_brackets)
{
    cr_expect_eq(eval_expr("(1+1)*(1+1)"), 4);
}

Test (eval_expr, one_plus_multiplication)
{
    cr_expect_eq(eval_expr("1+24*3"), 73);
}

Test (eval_expr, multiplication_redivided)
{
    cr_expect_eq(eval_expr("2*3/2"), 3);
}
